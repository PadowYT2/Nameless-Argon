...
<body>
	...
	<div class="main-content">
			...
			<div class="card">
				<div class="card-body">
					<section class="content">
						...
						<h3 style="margin-block-end: 30px;"><i class="fa fa-terminal" aria-hidden="true">
								{$CONSOLE}:</i>
							{$SERVER_NAME}</h3>
						{if count($CMD_BTN)}
						{foreach from=$CMD_BTN item=item key=key}
						<button type="button" onclick="cmdSend('{$item}')" class="btn-info btn-xs">{$key}</button>
						{/foreach}
						{/if}
		...
		<div id="fixed_btn" class="btn-group">
			<button class="btn btn-info fix_btn" type="button" data-toggle="modal" data-target="#add_modal">
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<form action="" method="POST">
							...
							<div class="form-group">
								<input type="hidden" name="token" value="{$TOKEN}">
								<input style="width: 100%;" type="submit" class="btn btn-info" value="Add">
							</div>
